    ...
    if request.user.is_authenticated():
        ...
        inventory = {
            "amethyst": Inventory.objects.filter(
                user=request.user,
                item__url="small-amethyst-crystal-piece",
                box=False,
                pending=False,
            ).count(),
            "aquamarine": Inventory.objects.filter(
                user=request.user,
                item__url="small-aquamarine-crystal-piece",
                box=False,
                pending=False,
            ).count(),
            "diamond": Inventory.objects.filter(
                user=request.user,
                item__url="small-diamond-crystal-piece",
                box=False,
                pending=False,
            ).count(),
            "emerald": Inventory.objects.filter(
                user=request.user,
                item__url="small-emerald-crystal-piece",
                box=False,
                pending=False,
            ).count(),
            "ruby": Inventory.objects.filter(
                user=request.user,
                item__url="small-ruby-crystal-piece",
                box=False,
                pending=False,
            ).count(),
            "sapphire": Inventory.objects.filter(
                user=request.user,
                item__url="small-sapphire-crystal-piece",
                box=False,
                pending=False,
            ).count(),
            "topaz": Inventory.objects.filter(
                user=request.user,
                item__url="small-topaz-crystal-piece",
                box=False,
                pending=False,
            ).count(),
        }
        return render(
            request,
            "world/quarry_page.html",
            {"crystal_claim": crystal_claim, "inventory": inventory},
        )
    else:
...
def purchase_dowsing_rod(request):
    if request.user.is_authenticated():
        if request.user.profile.points >= 10000:
            ...
            request.session[
                "error"
            ] = "You do not have enough points to buy a Basic Rod."
            return redirect(error_page)
    else:
        request.session["error"] = "You must be logged in to view this page."
        return redirect(error_page)
...
def upgrade_dowsing_rod(request):
    if request.user.is_authenticated():
        if request.user.profile.dowsing_rod == 1:
            ...
            request.session[
                "error"
            ] = "You must have a Rod to upgrade, and you cannot upgrade from a Best Rod."
            return redirect(error_page)
        ...
        if request.user.profile.points >= points:
            ...
            request.session[
                "error"
            ] = "You do not have enough points to upgrade your rod."
            return redirect(error_page)
    else:
        request.session["error"] = "You must be logged in to view this page."
        return redirect(error_page)
        ...
        if crystal_claim is None:
            if request.user.profile.dowsing_rod > 0:
                if request.user.profile.points >= 200:
                    ...
                    if (
                        ...
                        if get_card != 1:
                            ...
                            if rarity is not None:
                                ...
                                message = (
                                    "You try to find a crystal...and you find... a "
                                    + reward.name
                                    + "!"
                                )
                            ...
                            message = (
                                "You try to find a crystal...and you find... a... "
                                + reward.name
                                + "? That's kind of weird, someone must have dropped it..."
                            )
                            claim = DailyClaim.objects.create(
                                user=request.user,
                                daily_type="dowse",
                    ...
                    request.session[
                        "error"
                    ] = "You do not have enough points to go dowsing."
                    return redirect(error_page)
            else:
...
def trade_crystals(request, crystal):
    if request.user.is_authenticated():
        if (
            ...
            crystals_in_inventory = Inventory.objects.filter(
                user=request.user, item=crystal, box=False, pending=False
            )
            if crystals_in_inventory.count() >= 5:
                ...
                for crystal in crystals_in_inventory:
                    if n < 5:
                        ...
                        break
                new_crystal = Item.objects.get(
                    url="big-" + crystal_str + "-crystal-piece"
                )
                ...
                request.session[
                    "error"
                ] = "You do not have enough crystals to trade them in."
                return redirect(error_page)
        else:
