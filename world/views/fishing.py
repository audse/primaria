...
def fishing_page(request):
    if request.user.is_authenticated():
        ...
        inventory_fish = Inventory.objects.filter(
            user=request.user,
            item__category=fish_category,
            item__name__icontains="fish",
            box=False,
            pending=False,
        )
        return render(
            request,
            "world/fishing_page.html",
            {"fish_claim": fish_claim, "inventory_fish": inventory_fish},
        )
    else:
    ...
    if request.user.is_authenticated():
        ...
        if request.user.profile.points >= price:
            ...
            request.session["error"] = (
                "You do not have enough points to buy " + str(amount) + " bait."
            )
            return redirect(error_page)
    else:
        request.session["error"] = "You must be logged in to view this page."
        ...
        if fish_claim is None:
            if request.user.profile.fishing_bait > 0:
                if request.user.profile.fishing_rod > 0:
                    ...
                    fishing_skill_bonus = random.randint(
                        request.user.profile.fishing_skill, 20
                    )
                    ...
                    if (
                        ...
                        if rarity is not None:
                            reward = (
                                Item.objects.filter(
                                    rarity=rarity, category=fishing_category
                                )
                                .order_by("?")
                                .first()
                            )
                            ...
                            message = (
                                "You cast your line and... you pull up a "
                                + reward.name
                                + "!"
                            )
                            if rarity > 0:  # rarity 0 is junk...boot, tire, etc
                                if request.user.profile.fishing_skill < 10:
                                    ...
                                    if (
                                        request.user.profile.fishing_skill_progress
                                        == 10 * request.user.profile.fishing_skill
                                    ):
                            ...
                            if rand_num == 1:
                                penguin_avatar = Avatar.objects.get(
                                    url="saved-a-penguin"
                                )
                                if (
                                    penguin_avatar
                                    ...
                                    if inventory.item.url == "six-pack-rings":
                                        ...
                                        message = Message.objects.create(
                                            ...
                                            text='You have just received the avatar "Saved a Penguin" to use on the boards!',
                                        )
                ...
                request.session[
                    "error"
                ] = "You must have at least one bait to go fishing."
                return redirect(error_page)
        else:
...
def sell_fish(request, pk):
    if request.user.is_authenticated():
        ...
        fish = Inventory.objects.filter(
            user=request.user,
            pk=pk,
            item__category=fishing_category,
            item__name__icontains="fish",
        ).first()
        if fish:
