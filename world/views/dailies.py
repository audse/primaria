        ...
        if claims == 0:
            ...
            if gathered == "inspiration":
                ...
                if find_something:
                    ...
                    message = (
                        "You spend some time looking, and notice "
                        + str(points)
                        + " points on the ground. Thankfully no one else was observant enough to see you take them!"
                    )
                    ...
                    claim = DailyClaim.objects.create(
                        user=request.user, daily_type="garden", message=message
                    )
                        ...
                        if aesthetic_avatar not in request.user.profile.avatars.all():
                            if pet.color == "opalescent" or pet.color == "marble":
                                ...
                                message = Message.objects.create(
                                    ...
                                    text='You have just received the avatar "A E S T H E T I C" to use on the boards!',
                                )
                ...
                if find_something:
                    ...
                    message = (
                        "You look around in the bushes, and find a book ("
                        + book.name
                        + ") that was accidentally left here. No one notices when you slip it into your bag..."
                    )
                    claim = DailyClaim.objects.create(
                        user=request.user,
                        daily_type="garden",
                        ...
                        if thinker_avatar not in request.user.profile.avatars.all():
                            if inventory.item.second_category == Category.objects.get(
                                ...
                                message = Message.objects.create(
                                    receiving_user=request.user,
                                    subject="You just found a secret avatar!",
                                    text='You have just received the avatar "Thinker" to use on the boards!',
                                )
                    else:
                        message += (
                            "But your bag is full! You put it back where you found it."
                        )
                    return redirect(garden_page)
                else:
            ...
            request.session[
                "error"
            ] = "You have already gathered from the rose garden today."
...
def almighty_meatloaf_page(request):
    if request.user.is_authenticated():
        ...
        loaf_claim = None
    return render(
        request, "world/almighty_meatloaf_page.html", {"loaf_claim": loaf_claim}
    )
        ...
        if loaf_claim is None:
            ...
            if (
                ...
                if loaf_avatar not in request.user.profile.avatars.all():
                    loaves = Inventory.objects.filter(
                        user=request.user,
                        box=False,
                        pending=False,
                        item__name__icontains="meatloaf",
                    ).count()
                    if loaves >= 25:
                        ...
                        message = Message.objects.create(
                            receiving_user=request.user,
                            subject="You just found a secret avatar!",
                            text='You have just received the avatar "Loafing Around" to use on the boards!',
                        )

                if take_chance != 1:
                    reward = (
                        Item.objects.filter(
                            category=Category.objects.get(name="meatloaf")
                        )
                        .order_by("?")
                        .first()
                    )
                    message = (
                        "You approach the Almighty and snag a chunk of "
                        + reward.name
                        + " to take home."
                    )
                    ...
                    claim = DailyClaim.objects.create(
                        user=request.user, daily_type="meatloaf", message=message
                    )
                    return redirect(almighty_meatloaf_page)
            else:
                request.session[
            ...
            request.session[
                "error"
            ] = "You have already taken a chunk of meatloaf today."
            return redirect(error_page)
    else:
        request.session["error"] = "You must be logged in to view this page."
        return redirect(error_page)
...
def trails_page(request):
    if request.user.is_authenticated():
        ...
        trails_claim = None
    return render(
        request, "world/trails_page.html", {"pet": pet, "trails_claim": trails_claim}
    )
        ...
        if pet:
            ...
            if trails_claim is None:
                ...
                if stat == 1:  # wellness
                    ...
                    message = (
                        "You run around the trails with "
                        + pet.name
                        + " and they seem to be feeling healthier already!"
                    )
                    claim = DailyClaim.objects.create(
                        user=request.user, daily_type="trails", message=message
                    )
                    ...
                    message = (
                        "You run around the trails with "
                        + pet.name
                        + " and they seem completely rejuvinated!"
                    )
                    claim = DailyClaim.objects.create(
                        user=request.user, daily_type="trails", message=message
                    )
                    ...
                    message = (
                        "You run around the trails with "
                        + pet.name
                        + " but they didn't seem to into it today."
                    )
                    claim = DailyClaim.objects.create(
                        user=request.user, daily_type="trails", message=message
                    )
                ...
                request.session[
                    "error"
                ] = "Your pet is too tired to run with you on the trails again today."
                return redirect(error_page)
        else:
...
def apple_orchard_page(request):
    if request.user.is_authenticated():
        ...
        orchard_claim = None
    return render(
        request, "world/daily/apple_orchard_page.html", {"orchard_claim": orchard_claim}
    )
    ...
    if request.user.is_authenticated():
        ...
        if orchard_claim == None:
            if (
                ...
                if take_chance != 8:
                    reward = (
                        Item.objects.filter(url__icontains="apple")
                        .order_by("?")
                        .first()
                    )
                    ...
                    claim = DailyClaim.objects.create(
                        user=request.user, daily_type="orchard", message=message
                    )
                    return redirect(apple_orchard_page)
            else:
                request.session[
...
def day_old_stock_page(request):
    if request.user.is_authenticated():
        ...
        dayold_claim = None
    return render(
        request, "world/daily/day_old_stock_page.html", {"dayold_claim": dayold_claim}
    )
    ...
    if request.user.is_authenticated():
        ...
        if dayold_claim == None:
            if (
                ...
                reward = (
                    Item.objects.filter(url__icontains="bread").order_by("?").first()
                )
