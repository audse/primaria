...
def your_shop_page(request):
    if request.user.is_authenticated():
        ...
        return render(
            request,
            "shop/your_shop_page.html",
            {"shop": shop, "shop_items": shop_items, "user_items": user_items},
        )
    else:
    ...
    if request.user.is_authenticated():
        ...
        if shop is None:
            if request.user.profile.points >= 500:
                ...
                shop = UserShop.objects.create(
                    user=request.user, name=request.POST.get("shop_name")
                )
                return redirect(your_shop_page)
            else:
                request.session[
                    "error"
                ] = "You do not have enough points to open a shop."
                return redirect(error_page)
        else:
            request.session["error"] = "You have already created a shop."
            return redirect(error_page)
            ...
            if inventory_item:
                ...
                if item_count < shop.space:
                    ...
                    new_shop_item = UserShopStock.objects.create(
                        item=inventory_item.item, price=price, shop=shop
                    )
                ...
                if shop_item:
                    ...
                    if num_items_in_inventory < 50:
                        inventory_item = Inventory.objects.create(
                            user=request.user, item=shop_item.item
                        )
                        shop_item.delete()
                    else:
                        request.session[
    ...
    if request.user.is_authenticated():
        ...
        if shop:
            ...
            if withdraw_amount <= shop.shop_till:
                ...
                request.session[
                    "error"
                ] = "You cannot withdraw more than you have in your shop till."
                return redirect(error_page)
        else:
            request.session["error"] = "You do not yet have a shop."
            return redirect(error_page)
    ...
    if request.user.is_authenticated():
        ...
        if shop:
            if request.user.profile.points >= shop.upgrade_cost:
                ...
                request.session[
                    "error"
                ] = "You do not have enough points to upgrade your shop."
                return redirect(error_page)
        else:
...
def user_shop_page(request, username):
    ...
    if user:
        ...
        if shop:
            ...
            return render(
                request,
                "shop/user_shop_page.html",
                {"username": username, "shop": shop, "shop_items": shop_items},
            )
        else:
            ...
            if shop:
                ...
                if shop_item:
                    ...
                    if request.user.profile.points >= shop_item.price:
                        ...
                        request.session[
                            "error"
                        ] = "You do not have enough points to buy that item."
                        return redirect(error_page)
