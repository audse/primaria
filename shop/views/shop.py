...
def shop_search_results_page(request):
    ...
    if keyword is not None and keyword != "":
        ...
        user_search_results = UserShopStock.objects.filter(item__url__contains=new_keyword).order_by('?')[:10]

        search_results = sorted(chain(shop_search_results, user_search_results), key=lambda o: o.price)
        
        return render(request, 'shop/shop_search_results_page.html', {'keyword':keyword, 'search_results':search_results})
    else:
