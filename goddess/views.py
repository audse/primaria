...
def goddess_neglect_page(request):
    if request.user.is_authenticated:
        ...
        unfinished_quest = Quest.objects.filter(
            user=request.user, goddess="neglect", completed=False
        ).first()
        quests_today = Quest.objects.filter(
            user=request.user,
            goddess="neglect",
            completed=True,
        ...
        return render(
            request,
            "goddess/neglect_page.html",
            {
                "pet": pet,
                "maximum": maximum,
                "claims": claims,
                "unfinished_quest": unfinished_quest,
...
def goddess_sun_page(request):
    if request.user.is_authenticated:
        ...
        unfinished_quest = Quest.objects.filter(
            user=request.user, goddess="sun", completed=False
        ).first()
        quests_today = Quest.objects.filter(
            user=request.user,
            goddess="sun",
...
def goddess_garden_page(request):
    if request.user.is_authenticated:
        ...
        unfinished_quest = Quest.objects.filter(
            user=request.user, goddess="garden", completed=False
        ).first()
        quests_today = Quest.objects.filter(
            user=request.user,
            goddess="garden",
...
def goddess_ocean_page(request):
    if request.user.is_authenticated:
        ...
        unfinished_quest = Quest.objects.filter(
            user=request.user, goddess="ocean", completed=False
        ).first()
        quests_today = Quest.objects.filter(
            user=request.user,
            goddess="ocean",
...
def goddess_commerce_page(request):
    if request.user.is_authenticated:
        ...
        unfinished_quest = Quest.objects.filter(
            user=request.user, goddess="commerce", completed=False
        ).first()
        quests_today = Quest.objects.filter(
            user=request.user,
            goddess="commerce",
        ...
        activated_card = Inventory.objects.filter(
            item=activated_card, user=request.user, box=False, pending=False
        ).first()

        return render(
            request,
            "goddess/commerce_page.html",
            {
                "unfinished_quest": unfinished_quest,
                "quests_today": quests_today,
                "completed": completed,
                ...
                if pet.hunger >= 3 and pet.wellness >= 4 and pet.happiness >= 3:
                    ...
                    if (
                        ...
                        if prize_type == 1:  # random stat potion
                            reward = (
                                Item.objects.filter(
                                    second_category=Category.objects.get(
                                        name="stat potion"
                                    )
                                )
                                .order_by("?")
                            ...
                            message = (
                                "Thank you for keeping your pet happy and healthy! You have received a "
                                + reward.name
                                + " as a reward."
                            )
                            inventory = Inventory.objects.create(
                                user=request.user, item=reward
                            )
                            ...
                            reward = (
                                Item.objects.filter(
                                    category=Category.objects.get(name="medicine")
                                )
                                .order_by("?")
                                .first()
                            )
                            ...
                            reward = (
                                Item.objects.filter(
                                    category=Category.objects.get(name="plush"),
                                    rarity__gte=2,
                                )
                                .order_by("?")
                                .first()
                            ...
                            prize_type == 4
                        ):  # random 3 items (hunger, wellness, happiness)
                            if (
                                Inventory.objects.filter(
                                    user=request.user, box=False, pending=False
                                ).count()
                                ...
                                wellness_reward = (
                                    Item.objects.filter(
                                        category=Category.objects.get(name="herbal"),
                                        rarity__lte=2,
                                    )
                                    .order_by("?")
                                    .first()
                                ...
                                happiness_reward = (
                                    Item.objects.filter(
                                        category=Category.objects.get(name="book"),
                                        rarity=1,
                                    )
                                    .order_by("?")
                                    .first()
                                ...
                                message = (
                                    ...
                                    + hunger_reward.name
                                    + ", a "
                                    + wellness_reward.name
                                    + ", and a "
                                    + happiness_reward.name
                                    + " as a reward."
                                )
                                inventory = Inventory.objects.create(
                                    user=request.user, item=hunger_reward
                                )
                                inventory = Inventory.objects.create(
                                    user=request.user, item=wellness_reward
                                )
                                inventory = Inventory.objects.create(
                                    user=request.user, item=happiness_reward
                                )
                                claim = DailyClaim.objects.create(
                                    user=request.user,
                                    daily_type="neglect",
                    ...
                    request.session[
                        "error"
                    ] = "You have already claimed your gift for today."
                    return redirect(error_page)
            else:
                request.session[
                    "error"
                ] = "Your pet must be at maximum stats to receive a gift from the Goddess of Neglect."
                return redirect(error_page)
        else:
    ...
    if request.user.is_authenticated:
        ...
        if unfinished_quests < 1:
            if quests_today < 5:
                ...
                if difficulty == 1:
                    requested_item = (
                        Item.objects.exclude(category=potion_category)
                        .exclude(category=crystal_category)
                        .exclude(category=area_card_category)
                        .filter(rarity=1)
                        .order_by("?")
                        .first()
                    )
                ...
                if prize_type == 1:  # only item
                    if goddess != "sun":
                        reward_item = (
                            Item.objects.filter(category=item_category, rarity=2)
                            .order_by("?")
                            .first()
                        )
                        reward_points = None
                    ...
                    if goddess != "sun":
                        reward_item = (
                            Item.objects.filter(category=item_category, rarity=1)
                            .order_by("?")
                            .first()
                        )
                    else:
                ...
                request.session[
                    "error"
                ] = "You have already completed the maximum quests for this Goddess today."
                return redirect(error_page)
        else:
            request.session[
                "error"
            ] = "You cannot start a new quest if you haven't completed or cancelled the current one."
            return redirect(error_page)
    else:
        request.session["error"] = "You must be logged in to view this page."
        return redirect(error_page)
    ...
    if request.user.is_authenticated:
        ...
        if quest:
            item = Inventory.objects.filter(
                user=request.user, item=quest.requested_item, box=False, pending=False
            ).first()
            if item:
                if quest.reward_item:
                    reward_item = Inventory.objects.create(
                        user=request.user, item=quest.reward_item
                    )
                ...
                if divine_avatar not in request.user.profile.avatars.all():
                    new_quest_count = Quest.objects.filter(
                        user=request.user,
                        completed=True,
                        cancelled=False,
                        date__year=today.year,
                    ...
                    if new_quest_count >= 20:
                        ...
                        message = Message.objects.create(
                            ...
                            text='You have just received the avatar "Divine" to use on the boards!',
                        )

                request.session[
                    "completed"
                ] = "You have completed your quest! Check your inventory and/or points for your reward."
                if goddess == "neglect":
                    return redirect(goddess_neglect_page)
                elif goddess == "garden":
                    return redirect(goddess_garden_page)
                ...
                request.session[
                    "error"
                ] = "You do not have the item you need to complete your quest."
                return redirect(error_page)
        else:
