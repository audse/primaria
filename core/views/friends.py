...
def send_friend_request(request, username):
    ...
    if request.user in receiving_user.profile.blocked_users.all() or request.user.profile.disable_friend_requests:
        return handle_error(request, 'Sorry, you are unable to send a friend request to this user.')
    ...
    if check_for_previous_request or check_for_pending_request:
        return handle_error(request, 'You have already sent a friend request to this user (or they have already sent one to you).')
    ...
    if note:
        text = request.user.username + " sent you a friend request with the following note: \n " + note
    else:
...
def accept_friend_request(request, username):
    ...
    friend_request = FriendRequest.objects.filter(
            sending_user__username=username,
            receiving_user__username=request.user.username
        ).first()
